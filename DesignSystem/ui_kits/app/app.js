...
(function () {
  ...
  var STEPS = [
    { title: "Définir le projet", state: "done", when: "10 avril",
      fields: [["Projet", "Achat résidence principale"], ["Secteur", "Lyon 3e / 6e"]],
      cond: { text: "Projet cadré", when: "10 avr." } },
    { title: "Estimer ma capacité d’emprunt", state: "done", when: "28 avril",
      fields: [["Apport", "60 000 €"], ["Capacité estimée", "≈ 320 000 €"]],
      doc: { name: "Attestation de financement.pdf", meta: "PDF · 180 Ko" },
      cond: { text: "Capacité confirmée", when: "28 avr." } },
    { title: "Visiter & faire une offre", state: "done", when: "12 mai",
      fields: [["Bien", "Appartement · Lyon 3e · 68 m²"], ["Offre", "315 000 €"]],
      doc: { name: "Offre d’achat.pdf", meta: "Signée · 240 Ko" },
      cond: { text: "<b>Offre acceptée</b> par le vendeur", when: "12 mai" } },
    { title: "Signer le compromis de vente", state: "current",
      desc: "J’ai préparé le compromis avec le notaire. Relisez-le tranquillement — il ne reste qu’à confirmer le rendez-vous.",
      time: "J-12", timeClass: "ts-time--soon", pay: "~1 200 €", tag: "Notaire", provider: true,
      condDone: { text: "<b>Compromis signé</b>", when: "auj." } },
    { title: "Accord de prêt de la banque", state: "waiting",
      reason: "Ce n’est pas à vous de jouer — je relance la banque.", detail: "~2 semaines" },
    { title: "Préparer les fonds chez le notaire", state: "locked",
      reason: "Disponible après le compromis",
      desc: "Je vous indiquerai le montant exact et le RIB du notaire. Rien à préparer avant.",
      time: "~2 semaines", tag: "Notaire",
      condDone: { text: "<b>Fonds virés</b> au notaire", when: "auj." } },
    { title: "Signer l’acte définitif", state: "locked",
      ...
      time: "Courant juin", tag: "Notaire",
      condDone: { text: "<b>Acte authentique signé</b>", when: "auj." } },
    { title: "Récupérer les clés 🔑", state: "locked",
  ...
  function below(i) { return STEPS[i].state === "done" ? "taken" : "future"; }

  function condPill(c) {
    if (!c) return "";
    return '<p class="ts-cond"><span class="ts-cond__check">' + ICON_CHECK + "</span> " + c.text +
      (c.when ? ' <span class="ts-cond__when">' + c.when + "</span>" : "") + "</p>";
  }
  ...
  function recordBody(s) {
    ...
    if (s.doc) {
      parts += '<div><span class="ts-record__sub">Livrable</span><div class="ts-docs" style="margin-top:6px">' +
        '<a class="ts-doc" href="#"><span class="ts-doc__icon">' + ICON_DOC + '</span><span><span class="ts-doc__name">' +
        s.doc.name + '</span><span class="ts-doc__meta">' + s.doc.meta + "</span></span></a></div></div>";
    }
    return parts;
  }

  function renderItem(s, i) {
    var rail = '<span class="ts-trail__rail"><span class="ts-glyph" data-state="' +
      (s.state === "current" ? "current" : s.state) + '"></span></span>';

    if (s.state === "done") {
      var hasBody = (s.fields && s.fields.length) || s.doc;
      var rec = '<details class="ts-record"' + (i === 2 ? " open" : "") + '>' +
        '<summary class="ts-record__head"><span class="ts-record__titles">' +
        '<p class="ts-record__title">' + s.title + '</p>' +
        '<span class="ts-record__when">Fait le ' + (s.when || "—") + '</span></span>' +
        (hasBody ? ICON_CHEV : "") + "</summary>" +
        (hasBody ? '<div class="ts-record__body">' + recordBody(s) + "</div>" : "") +
        "</details>";
      return '<li class="ts-trail__item" data-state="done" data-below="' + below(i) + '">' +
        rail + rec + condPill(s.cond) + "</li>";
    }
    ...
    if (s.state === "current") {
      ...
      var focal = '<div class="ts-focal">' +
        '<div class="ts-focal__eyebrow"><span class="eyebrow">Vous êtes ici · étape ' + (i + 1) + " / " + TOTAL + "</span></div>" +
        '<h3 class="ts-focal__title">' + s.title + "</h3>" +
        (s.desc ? '<p class="ts-focal__desc">' + s.desc + "</p>" : "") +
        (meta ? '<div class="ts-focal__meta">' + meta + "</div>" : "") +
        '<button class="ts-btn ts-btn--primary ts-btn--block js-done">C’est fait ' + ICON_CHECK + "</button>" +
        "</div>";
    ...
    var metaRow = "";
    if (s.state === "waiting") {
      metaRow = '<span class="ts-badge ts-badge--waiting">En attente</span>' +
        (s.detail ? '<span class="trail-detail">' + s.detail + "</span>" : "");
    } else {
      metaRow = '<span class="ts-trail__reason">' + (s.reason || "") + "</span>";
    }
    return '<li class="ts-trail__item" data-state="' + s.state + '" data-below="' + below(i) + '">' +
      rail + '<div class="ts-trail__body"><p class="ts-trail__title">' + s.title +
      '</p><div class="ts-trail__meta">' + metaRow + "</div></div></li>";
  }
  ...
  function renderHome() {
    ...
    if (i < 0) {
      if (card) {
        ...
        var p = document.getElementById("acProvider"); if (p) p.style.display = "none";
  ...
  function renderAll() { renderParcours(); renderHome(); setProgress(); }
  ...
  function go(name) {
    ...
    if (name === "plan") { renderParcours(); setTimeout(scrollToCurrent, 80); }
  }
  ...
  document.addEventListener("click", function (e) {
    ...
    if (b) { advance(); setTimeout(scrollToCurrent, 80); }
  });
