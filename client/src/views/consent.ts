...
export function openConsent(req: ConsentRequest): Promise<ConsentResult | null> {
  return new Promise(resolve => {
    ...
    const initialIndex = LEVELS.indexOf(level)
    ...
    dialog.innerHTML = `
      ...
      <div class="ts-disclosure" style="margin-top: var(--space-md)">
        ...
        <input class="ts-disclosure__range" type="range" min="0" max="2" step="1"
               value="${initialIndex}" aria-label="Niveau de partage des données"
               style="--_pct: ${initialPct}">
        <div class="ts-disclosure__ticks">
          ${LEVELS.map((l, i) => `
            <button type="button" class="ts-disclosure__tick ${i <= initialIndex ? "is-on" : ""}" data-level="${l}">
    ...
    const valuePill = dialog.querySelector<HTMLElement>(".ts-disclosure__value")!
    const desc = dialog.querySelector<HTMLElement>(".ts-disclosure__desc")!
    const impact = dialog.querySelector<HTMLElement>(".impact")!
    const fieldStack = dialog.querySelector<HTMLElement>(".field-stack")!
    const confirmBtn = dialog.querySelector<HTMLButtonElement>("[data-confirm]")!
    const ticks = Array.from(dialog.querySelectorAll<HTMLButtonElement>(".ts-disclosure__tick"))

    const applyLevel = (next: DossierSensitivity): void => {
      level = next
      const idx = LEVELS.indexOf(level)
      const pct = `${(idx / 2) * 100}%`
      valuePill.textContent = LEVEL_LABELS[level]
      range.style.setProperty("--_pct", pct)
      if (range.value !== String(idx)) range.value = String(idx)
      ticks.forEach((tick, i) => tick.classList.toggle("is-on", i <= idx))
      desc.textContent = LEVEL_DESC[level]
      impact.textContent = IMPACT[level]
      const transmitted = sharedFields(req.dossier, inputs, level)
      fieldStack.innerHTML = inputs.map(field => renderSharedField(field, req.dossier, level, transmitted)).join("")
      confirmBtn.disabled = transmitted.length === 0
    }

    range.addEventListener("input", () => {
      const idx = parseInt(range.value, 10)
      applyLevel(LEVELS[idx] ?? "contact")
    })
    ticks.forEach(tick => {
      tick.addEventListener("click", () => applyLevel(tick.dataset.level as DossierSensitivity))
    })
    dialog.querySelector<HTMLButtonElement>("[data-cancel]")?.addEventListener("click", () => finish(null))
    dialog.querySelector<HTMLButtonElement>("[data-close]")?.addEventListener("click", () => finish(null))
